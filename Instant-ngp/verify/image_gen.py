...
def picture_sample(images,LOS_dirs,omegas,image_hight = 105,image_width = 100, image_num = 37,temp_num = 0):
    # temp_num = random.sample(range(image_num),1)
    # temp_num = [0]
    temp_num = [temp_num]
    print(temp_num)
    temp = [t*image_hight for t in temp_num] + np.arange(105)
...
model.load_state_dict(torch.load('/DATA/disk1/Instant-ngp/model/experiment22/model_state_dict.pth'))
model.eval()

omega_real = math.pi/900

folder_path = '/DATA/disk1/Instant-ngp/dataset/real_data/real_data_reg_2024On_12'
...
for i in range(37):
    omegas_batch_tensor,LOS_dirs_batch_tensor,range_profile_batch_tensor,doppler_profil_num_tensor = picture_sample(images,LOS_dirs,omegas,image_hight = 105,image_num=37,temp_num=i)
    ...
    plt.axis('off')
    # 保存图片
    if os.path.exists('./image_gen3') == False:
        os.mkdir('./image_gen3')
    plt.savefig('./image_gen3/2024on'+str(i)+'.png', bbox_inches='tight', pad_inches=0)