...
def batchrender(omega,LOS,model,doppler_num):
    ...
    if code_flag == 1:
        ...
        LOS_coding = LOS
        LOS_coding = ((LOS_coding.unsqueeze(1).unsqueeze(2))*torch.ones(batch_size,distance_gap,n_gap,3).to(device)).view(-1,3)
...
# 载入数据
folder_path = './dataset/real_data/real_data_reg_2024On_13'
...
#载入模型
model = NeRF(input_ch = 63, input_ch_views = 3, use_viewdirs = True).to(device)

# # 指定预训练模型的路径
# pretrained_model_path = '/DATA/disk1/Instant-ngp/model/experiment20/model_state_dict.pth'  # 修改为您的预训练模型路径
