...
class NeRF(nn.Module):
    def __init__(self, D=4, W=64, input_ch=3, input_ch_views=3, output_ch=2, skips=[2], use_viewdirs=True):
        """
        D: 深度，多少层网络
        W: 网络内的channel 宽度
        input_ch: xyz的宽度
...
class field_model(nn.Module):
    ...
    def forward(self, xyz):
        ...
        model.load_state_dict(torch.load('/DATA/disk1/Instant-ngp/model/experiment4/model_state_dict.pth'))
        model.eval()
...
def main():
    ...
    # 检查 scalar_field 的值范围
    scalar_min, scalar_max = scalar_field.min(), scalar_field.max()
    print(f"Scalar field range: min={scalar_min}, max={scalar_max}")

    # 确保 threshold 在范围内
    if not (scalar_min <= threshold <= scalar_max):
        raise ValueError(f"Threshold {threshold} is out of range. Please choose a value between {scalar_min} and {scalar_max}.")
    ...
    with open(f"./code/pytorch3d-marching-cubes/marching_cube_result/result_{threshold}_{modelnum}.obj", "w") as f:
        for v in vertices:
