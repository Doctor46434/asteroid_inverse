...
def batchrender(omega,LOS,model,doppler_num):
    ...
    if render_equaltion == 0:
        ...
        # 将Ti的第1维首增加一个0，并去除最后一维，方便计算
        Ti = torch.cat((torch.zeros(batch_size,1,n_gap).to(device),Ti),dim=1)[:,:-1,:]
        distance_profile = torch.sum(output[:,:,:,0]*output[:,:,:,1]*n_delta*Ti,dim=2)
...
# 载入数据
folder_path = './dataset/sys_data/contact_ball/test01'

# 生成保存路径
experiment_name = 'experiment04'
...
# # 指定预训练模型的路径
# pretrained_model_path = '/DATA/disk1/Instant-ngp/model_unsurf/experiment35/model_state_dict.pth'  # 修改为您的预训练模型路径
