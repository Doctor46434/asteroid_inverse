...
def batchrender(LOS_angel,model,doppler_angle,xrange_max = 4):
    '''
    LOS_angel为一个[bs]变量，为雷达视线方向与目标水平面之间的夹角
    model是nerf模型，将一个已经进行位置编码后的位置和视线向量输入进model,可以返回这个位置的体积密度和散射系数
    doppler_num为一个[bs]变量，确定该条投影光线在Pol图像中的位置
    '''

    # 确定网格参数
    distance_max = xrange_max*torch.cos(LOS_angel) #[bs]
    distance_gap = 200
    doppler_gap = 360
    
    # 确定batch_size
    batch_size = LOS_angel.shape
    # 确定每个batch_size输入的投影平面
    # 多普勒向采样
    doppler_angel_rad = doppler_angle*math.pi/180
    doppler_vector = torch.stack([
        torch.cos(doppler_angel_rad),
        torch.sin(doppler_angel_rad),
        torch.zeros_like(doppler_angel_rad)
    ], dim=1).to(device)
    # 距离向采样
    distance = torch.linspace(0,1,distance_gap).to(device)
    distance = distance.unsqueeze(1)*distance_max
    distance_vector = torch.stack([
        -torch.cos(doppler_angel_rad)*torch.cos(LOS_angel),
        -torch.sin(doppler_angel_rad)*torch.cos(LOS_angel),
        -torch.sin(LOS_angel)
    ], dim=1).to(device)
    distance_map = distance.unsqueeze(2)*distance_vector.unsqueeze(1)   #[bs,200,3]
    distance_delta = distance_max/distance_gap  #[bs]
    # NSR向采样
    NSR_vector = torch.stack([
        -torch.cos(doppler_angel_rad)*torch.sin(LOS_angel),
        -torch.sin(doppler_angel_rad)*torch.sin(LOS_angel),
        -torch.cos(LOS_angel)
    ], dim=1).to(device)
...
args = parser.parse_args() # 从命令行参数中解析参数
...
def adjust_learning_rate(optimizer,epoch,lr = 5e-5):
...
for epoch in range(20001):
    ...
    if epoch % 2000 == 0:
        ...
        loss_dir = os.path.dirname(args.loss_path)
