...
def batchrender(omega,LOS,model,doppler_num):
    ...
    lambda0 = c/fc
    # 确定网格参数
    distance_max = args.max_distance      # 0.60
    distance_min = args.min_distance      # -0.60
    distance_gap = args.distance_interval # 100
    doppler_max = args.max_doppler        # 0.15
    doppler_min = args.min_doppler        # -0.15
    doppler_gap = args.doppler_interval   # 100
    n_max = args.max_nsr                  # 0.30
    n_min = args.min_nsr                  # -0.30
    n_gap = args.nsr_interval             # 120
    # 确定输入batch_size
    ...
    if args.random_distance == True:
        ...
        distance_map = LOS.unsqueeze(1)*distance_random_array.unsqueeze(2)
        distance_delta_average = torch.tensor((distance_max-distance_min)/distance_gap).to(device)
        distance_delta_temp = distance_random_array[:,1:]-distance_random_array[:,0:-1]
        distance_delta= torch.cat(
            [distance_delta_temp, distance_delta_average.expand(distance_delta.shape[0], 1)],
            dim=1
        )
    else:
        # 等间距对距离向采样
    ...
    # 多普勒采样
    if args.is_doppler == True:
        # 为距离-距离图片
        doppler = torch.linspace(doppler_min,doppler_max,doppler_gap).repeat(batch_size,1).to(device)
        doppler = doppler*((distance_max-distance_min)/(doppler_max-doppler_min))/LOSomega_sin_angel.unsqueeze(1)
        doppler_map = doppler.unsqueeze(2)*Doppler_vector.unsqueeze(1)
    else:
        # 为距离-多普勒图片
        doppler = torch.linspace(doppler_min,doppler_max,doppler_gap).repeat(batch_size,1).to(device)
        doppler = doppler*((distance_max-distance_min)/(doppler_max-doppler_min))*lambda0/omega_norm.unsqueeze(1)/2/LOSomega_sin_angel.unsqueeze(1)
        doppler_map = doppler.unsqueeze(2)*Doppler_vector.unsqueeze(1)
    ...
    # 计算所有需要输入网络的坐标
    if args.use_positional_encoding == True:
        xyz = doppler_map[torch.arange(batch_size),doppler_num,:].unsqueeze(1).unsqueeze(2) + distance_map.unsqueeze(2) + n_random_map
        xyz_max = distance_max**2 + (doppler_max*((distance_max-distance_min)/(doppler_max-doppler_min)))**2 + n_max**2
        xyz_max = torch.tensor(xyz_max).to(device)
        xyz_max = torch.sqrt(xyz_max)
        xyz_normalize = xyz/xyz_max
        xyz_coding = positon_code_xyz(xyz_normalize)
...
def picture_sample(images,LOS_dirs,omegas,img_height = 100,img_num = 60,num = 0):
    temp_num = random.sample(range(img_num),1)
    temp_num = [num]
    print(temp_num)
    temp = [t*img_height for t in temp_num] + np.arange(100)
    data_num = [x//img_height for x in temp]
    doppler_numbers = [x % img_height for x in temp]
...
args, unknown = parser.parse_known_args()  # 使用 parse_known_args() 来过滤多余的参数

device = torch.device("cuda:6" if torch.cuda.is_available() else "cpu")
print(torch.cuda.is_available())

model = NeRF(input_ch = 63, input_ch_views = 27, use_viewdirs = True).to(device)
model.load_state_dict(torch.load('/DATA/disk1/ISAR_NERF/ISAR_NeRF_program/model/model_2.pth'))
model.eval()
...
for i in range(37):
    omegas_batch_tensor,LOS_dirs_batch_tensor,range_profile_batch_tensor,doppler_profil_num_tensor = picture_sample(images,LOS_dirs,omegas,img_height = args.img_height,img_num=args.image_num,num=i)
    ...
    fig = plt.figure()
    plt.imshow(range_image1,cmap='gray')
    # 删除坐标轴
    plt.axis('off')
    # 保存图片
    if os.path.exists('./image2') == False:
        os.mkdir('./image2')
    plt.savefig('./image2/1620geo'+str(i)+'.png', bbox_inches='tight', pad_inches=0)
...
# axes[0].axis('off')
# # 将灰度图像保存为图片
# # plt.imsave('groundtruth1.png',range_image2,cmap='hot')
...
# axes[1].axis('off')
# # 将灰度图像保存为图片
# # plt.imsave('nerf2.png',range_image1,cmap='hot')
