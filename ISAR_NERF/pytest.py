...
def point_cloud_to_unit_vectors(point_cloud):
    """
    输入一个点云，返回点云上每个点从原点到该点的单位向量。

    参数:
    point_cloud (torch.Tensor): 形状为 (N, 3) 的张量，代表 N 个三维点。

    返回:
    torch.Tensor: 形状为 (N, 3) 的张量，代表 N 个单位向量。
    """
    # 计算点云中每个点从原点到该点的方向向量
    vectors = point_cloud
    
    # 计算每个向量的模长
    lengths = torch.norm(vectors, p=2, dim=1, keepdim=True)
...
def ISAR_imaging(time_start,vextex,normal,horizon,rotation_state,num):
    ...
    Tcoh = torch.tensor([10]).to(device)
    PRF = torch.tensor([20]).to(device)
    fc = torch.tensor([9.7e9]).to(device)
    Tp = torch.tensor([5e-4]).to(device)
    B = torch.tensor([30e6]).to(device)
    k = B/Tp
    fs  = 1.2*B
    Tr = 1/PRF
    ...
    # #可视化
    # vextex_visual = vextex.detach().cpu()
    # vextex_visual = vextex_visual.numpy()
    # rotation_axis_visual = rotation_axis.detach().cpu()
    # rotation_axis_visual = rotation_axis_visual.numpy()
    # rotation_axis_visual = rotation_axis_visual * 100
    # fig = plt.figure()
    # ax = fig.add_subplot(111, projection='3d')
    # ax.scatter(vextex_visual[:,0], vextex_visual[:,1], vextex_visual[:,2], c='r', marker='o')
    # ax.scatter(0, 0, 0, c='b', marker='o')
    # ax.quiver(0,0,0, rotation_axis_visual[0], rotation_axis_visual[1], rotation_axis_visual[2], color='r')
    # plt.xlabel('x')
    # plt.ylabel('y')
    # plt.axis('equal')
    # plt.show()
    ...
    # 方位向分辨率
    dopple_res = lambda0/2/theta
    [Range_map,Dopple_map] = torch.meshgrid(torch.linspace(-60,60,100).to(device),torch.linspace(-15,15,100).to(device), indexing='xy')
    z = torch.zeros_like(Range_map).to(device)
    z = z.unsqueeze(0).expand(vextex.shape[1],-1,-1)
    z = torch.abs(sigma.unsqueeze(1).unsqueeze(2)*sinc_windowed(1/range_res*(Range_map.unsqueeze(0)-Range_domain.unsqueeze(1).unsqueeze(2)),fs_range_window,N_range_window)*sinc_windowed(1/dopple_res*(Dopple_map.unsqueeze(0)-Dopple_domian.unsqueeze(1).unsqueeze(2)),fs_dopple_window,N_dopple_window))
    z = torch.sum(z,dim=0)
    ...
    print(LOS_real)
    print(num)
    filename = './asteroid_image/wx_multiline_300_highres_diffomega/Nerf_data' + str(num)

    np.savez(filename, image=z, LOS = LOS_real, rotation_axis = rotation_axis)
    # lambda0 = lambda0.detach().cpu()
    # x_bot = 15*lambda0/math.pi*900/2
    # x_bot = x_bot.numpy()
    # extent = [-60, 60, -x_bot, x_bot]
...
# 输入已知模型
vextex = read_obj_file('./asteroid_model/wx_highres.obj')
vextex = vextex *20
...
## 返回简单法向量
normals = point_cloud_to_unit_vectors(vextex)
# 旋转时间
rotation_time = torch.linspace(0,1800,100).to(device)
# 视线方向
LOS_dir = torch.tensor([[-1,0,0],[-math.sqrt(3)/2,0,1/2],[-math.sqrt(3)/2,0,-1/2]]).to(device)
# LOS_dir = torch.tensor([[0,1,0],[-1,0,0],[-math.sqrt(3)/2,0,1/2]]).to(device)
# 物体转动周期与转轴指向
rotation_state = torch.tensor([[0.5,0,math.pi/6],[0.5,0,0],[0.5,0,-math.pi/6]]).to(device)

for j in range(1):
    for k in range(3):
        for i in range(100):
            z = ISAR_imaging(rotation_time[i], vextex, normals, LOS_dir[j,:].repeat(200,1).float().to(device), rotation_state[k,:],k*100+i)