...
class NeRF(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            ...
            alpha = 1-torch.exp(-self.relu(alpha)) 
...
model.load_state_dict(torch.load('model_state_dict17.pth'))
model.eval()

LOS_dir = torch.tensor([math.sqrt(3)/2,0,-1/2]).float().to(device).repeat(100,1)
omega = torch.tensor([0,0,1]).float().to(device).repeat(100,1)
doppler = torch.arange(100).to(device)
omega_real = math.pi/900

distance_profile_batch = batchrender(omega*omega_real,LOS_dir,model,doppler)

distance_profile_batch = distance_profile_batch.detach().cpu()
distance_profile_batch = distance_profile_batch.numpy()

plt.imshow(distance_profile_batch)
plt.colorbar()
plt.show()
