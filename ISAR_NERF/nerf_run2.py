...
def batchrender(omega,LOS,model,doppler_num):
    ...
    distance = distance_array[:,0:-1] + (distance_array[:,1:]-distance_array[:,0:-1])*torch.rand(batch_size,distance_gap).to(device)
...
def loaddata(folder_path):
    ...
    print("Data shape:",len(LOS_dirs))
    print("Data shape:",len(omegas))
...
def random_sample(images,LOS_dirs,omegas,batch_size):
    temp = random.sample(range(6000), batch_size)
...
# 载入数据
folder_path = '/DATA/disk1/3dmodel/3dmodel/ISAR_NERF/asteroid_image_nerf_new/luoxuan_new'
images,LOS_dirs,omegas = loaddata(folder_path)

#载入模型
model = NeRF(input_ch = 3, input_ch_views = 3, use_viewdirs = True).to(device)
...
for epoch in range(20000):
    ...
    if epoch % 2000 == 0:
        torch.save(model.state_dict(), 'model_state_dict93.pth')
        torch.save(losses, 'loss_list93.pth')
    loss.backward()
    optimizer.step()
