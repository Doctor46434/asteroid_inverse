...
def random_sample(images,LOS_dirs,omegas,batch_size):
    temp = random.sample(range(29900), batch_size)
...
# 载入数据
folder_path = '/DATA/disk1/3dmodel/3dmodel/ISAR_NERF/asteroid_image_nerf_new/-5db_denoise_submean'
...
for epoch in range(20000):
    ...
    if epoch % 2000 == 0:
        torch.save(model.state_dict(), 'model_state_dict84.pth')
        torch.save(losses, 'loss_list84.pth')
    loss.backward()
    optimizer.step()
