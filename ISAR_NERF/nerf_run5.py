...
def random_sample(images,LOS_dirs,omegas,batch_size):
    temp = random.sample(range(9000), batch_size)
...
# 载入数据
folder_path = '/DATA/disk1/3dmodel/3dmodel/ISAR_NERF/asteroid_image_nerf_new/luoxuan1-30'
...
for epoch in range(20000):
    ...
    if epoch % 2000 == 0:
        torch.save(model.state_dict(), 'model_state_dict81.pth')
        torch.save(losses, 'loss_list81.pth')
    loss.backward()
    optimizer.step()
